...
<v-container class="grey lighten-3" :fill-height="loading">
    <v-row justify="center" align="center" class="py-auto my-auto">
        <v-col md="12" lg="8" cols="12" class="my-0 py-0">
            <v-text-field dense prepend-inner-icon="mdi-magnify" class="py-0 my-0" solo v-model="search" @keyup="filtrer" placeholder="Chercher un projet" rounded></v-text-field>
        </v-col>
                ...
                <v-card-text>
                    <template v-if="!search">
                        <v-row justify="space-around" class="px-lg-16 flex-wrap" v-if="projets.length">
                            <v-col md="6" cols="12" v-for="(projet, i) in displayedProjects" :key="i">
                                <v-card>
                                    <v-card-title>
                            ...
                            projet.tache_array.length > 1 ? "s" : ""
                              ...
                              Math.floor(projet.buffer - projet.progression)
                                            ...
                                            <v-progress-linear striped stream :buffer-value="projet.buffer" rounded color="success" background-color="warning lighten-3" strip :value="projet.progression"></v-progress-linear>
                                        </div>
                                    </v-card-text>
                        ...
                        <v-row justify="center" class="fill-height" v-else>
                            <v-col cols="auto" class="mx-lg-16 text-center">
                                <v-alert color="grey lighten-3">
                                    Aucun projet trouvé
                                </v-alert>
                                <v-img :max-width="400" :src="require('../assets/empty.svg')" />
                    ...
                    <template v-else>
                        <v-row justify="start" class="px-lg-16 flex-wrap" v-if="projetsListFilter.length">
                            <v-col md="6" cols="12" v-for="(projet, i) in projetsListFilter" :key="i">
                                <v-card>
                                    <v-card-title>
                                        <v-row justify="space-between" class="px-4">
                                            <h5>{{ projet.nom }}</h5>
...
export default {
    ...
    methods: {
        filtrer() {
            this.projetsListFilter = this.projets.filter(p => p.nom.includes(this.search));
        },
        chipColor(status) {
